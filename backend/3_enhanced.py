    ...
    def get_context(self, query: str) -> str:
        ...
        try:
            ...
            gemini_prompt = (
                ...
                f"Limit your response to 1800 characters. Avoid generalities and focus on actionable, technical details.\n\nQUERY: {query}\n\nRESPONSE:")
...
def build_security_prompt(query, vuln_type=None):
    ...
    logger.info(f"[PROMPT] Full prompt sent to GPT4All:\n{'='*30}\n{prompt[:1200]}{'...truncated...' if len(prompt) > 1200 else ''}\n{'='*30}")
    return prompt

def generate_response_with_gpt4all(prompt, max_tokens=1500):
    ...
    try:
        response = llm.generate(
            prompt,
            max_tokens=3000,  # Increased from 1000 to 3000 for longer outputs
            temp=0.7,
            top_p=0.9,
            top_k=40
        )
...
def stream_chat():
    ...
    try:
        ...
        # Always use Gemini context as the primary source for all queries
        # If the detected type is in the dict, enrich with extra details, else just use Gemini context
        prompt = build_security_prompt(user_message, vuln_type if vuln_type in VULNERABILITIES else None)
        ai_response = generate_response_with_gpt4all(prompt)
        
        # If enrichment is available, format the response with extra details
        if vuln_type in VULNERABILITIES:
            ...
            source = "gemini+db+gpt4all"
        else:
            # For all other vulnerabilities (including all 6400+), use Gemini context as the main source
            response = ai_response or "[ERROR] No response generated."
            source = "gemini+gpt4all"
