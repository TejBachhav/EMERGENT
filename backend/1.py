...
- OWASP guidelines
- Examples and remediations
- Additional recommendations for improving security.
"""
            classification = "general"
        ...
        # Refine truncation logic to ensure prompt fits within the context window
        max_context_window = 2048
        prompt_tokens = len(prompt.split())

        if prompt_tokens > max_context_window:
            excess_tokens = prompt_tokens - max_context_window
            history_tokens = sum(len(msg['content'].split()) for msg in history)

            # Remove excess tokens from history while prioritizing user input
            while excess_tokens > 0 and history:
                removed_msg = history.pop(0)
                excess_tokens -= len(removed_msg['content'].split())

            # Rebuild the prompt with truncated history
            prompt = f"""
You are a security assistant helping developers understand vulnerabilities.
...
"""
        ...
        # Fallback mechanism for excessive prompt length
        if len(prompt.split()) > max_context_window:
            prompt = f"""
User query: "{user_input}"

The prompt exceeded the context window. Please refine your query or reduce history length.
"""

        # Directly transfer the user query to the model without additions
        full_response = llm.generate(prompt, max_tokens=1500, temp=0.5)

        # Update session and log BEFORE streaming
        history.append({"role": "user", "content": user_input})
        history.append({"role": "assistant", "content": full_response})
        session["conversation"] = history
        ...
        # Return a generator that yields one character at a time
        def generate_stream():
            for char in full_response:
                yield char
